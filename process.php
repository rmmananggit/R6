<?php
    // Start session_start(); -> This will initialize session. It is essential to use session for us to use $_SESSION variables. To store and retrieve session-related data. Since we're using $_SESSION, well import session_start();
        session_start();
    // Start session_start();

    // Start connecting config so that functions will run.
        include("config.php");
    // End connecting config so that functions will run.

    // Start regsiterButton Function
    ...
    // End registerButton Function

    // Start loginButton Function
    ...
    // Start handleDelete 
        if(isset($_POST["handleDelete"])) {
            ...
            // Start delete query from students
                $delete_student_query = "DELETE FROM `student` WHERE `studentID` = '$studentID'"; //I base the student to find instead of ID, since it is already been checked in register. You cannot register an existing studentID
